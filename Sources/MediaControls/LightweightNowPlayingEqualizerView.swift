...
/// CALayer-backed equalizer ported from DynamicNotch, trimmed for our
/// single-target use (macOS 26 only, borderless nonactivating panel).
/// 5 bars, scale-Y animated via `CABasicAnimation` on a 0.3s
/// ease-in-out cadence at 24fps preferred frame rate. NSView wrapper
/// because CALayer animations are way smoother + cheaper than driving
/// a SwiftUI TimelineView at 30Hz with manual sine math.
///
/// Occlusion: stops the animation timer when the hosting window
/// becomes occluded so the equalizer doesn't churn CPU while hidden.
...
private extension LightweightNowPlayingEqualizerNSView {
    ...
    func resetBars() {
        ...
        CATransaction.setDisableActions(true)
        for (index, barLayer) in barLayers.enumerated() {
            barLayer.removeAnimation(forKey: "notchApp.scaleY")
    ...
    func animateBars() {
        ...
        for (index, barLayer) in barLayers.enumerated() {
            ...
            // macOS 13+ unconditionally — we ship macOS 26.
            animation.preferredFrameRateRange = CAFrameRateRange(
                minimum: 24,
                maximum: 24,
                preferred: 24
            )
            barLayer.add(animation, forKey: "notchApp.scaleY")
        }
    }
    ...
    /// Only the occlusion notification matters — our borderless,
    /// nonactivating panels can never be miniaturized, so the
    /// didMiniaturize / didDeminiaturize / willClose observers from
    /// DynamicNotch's original code are dead weight here.
    func configureOcclusionObserver() {
        removeOcclusionObserver()
