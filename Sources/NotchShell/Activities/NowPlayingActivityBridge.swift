...
final class NowPlayingActivityBridge: ObservableObject {
    ...
    enum Event: Equatable {
        case started(NowPlayingActivity.Snapshot)
        case updated(NowPlayingActivity.Snapshot)
        case stopped
    }
    ...
    private func recompute() {
        ...
        if !wasActive {
            ...
            // Same track, refreshed payload (artwork landed, play/pause
            // toggled, etc.). In-place update.
            events.send(.updated(snapshot))
        }
    }
}
